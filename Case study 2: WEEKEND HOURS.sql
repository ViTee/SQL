...
HAVING DATEPART(W,TIMESTAMP) IN ('7','1')),
T AS (SELECT EMP_ID,TIMESTAMP,WK,ROW_NUMBER() OVER (PARTITION BY EMP_ID,M,D ORDER BY EMP_ID,M,D) AS STT,D,M
FROM CTE),
FT AS (SELECT T1.EMP_ID, DATEDIFF(HH,T1.TIMESTAMP,T2.TIMESTAMP) AS WH
FROM T T1 INNER JOIN T T2 ON
T1.WK=T2.WK AND T1.D=T2.D AND T1.M=T2.M
WHERE T1.STT=1 AND T2.STT=2) 
SELECT EMP_ID, SUM(WH) AS WORK_HOURS
FROM FT
GROUP BY EMP_ID
